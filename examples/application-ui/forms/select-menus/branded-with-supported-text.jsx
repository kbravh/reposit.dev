    ...
    <Listbox value={selected} onChange={setSelected}>
      ...
      <div className="relative">
        <div className="inline-flex divide-x divide-indigo-700 rounded-md outline-hidden">
          ...
          <ListboxButton className="inline-flex items-center rounded-r-md rounded-l-none bg-indigo-600 p-2 outline-hidden hover:bg-indigo-700 focus-visible:outline-2 focus-visible:outline-indigo-400">
        ...
        <ListboxOptions
          transition
          className="absolute right-0 z-10 mt-2 w-72 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-hidden data-closed:data-leave:opacity-0 data-leave:transition data-leave:duration-100 data-leave:ease-in"
        >
          {publishingOptions.map(option => (
            <ListboxOption
              key={option.title}
              value={option}
              className="group cursor-default select-none p-4 text-gray-900 text-sm data-focus:bg-indigo-600 data-focus:text-white"
            >
