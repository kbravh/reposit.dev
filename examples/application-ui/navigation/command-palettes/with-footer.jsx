			...
			<div className="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
				...
				>
					...
					>
						...
						{(filteredProjects.length > 0 || filteredUsers.length > 0) && (
							...
							>
								{filteredProjects.length > 0 && (
									<li>
										<h2 className="font-semibold text-gray-900 text-xs">
											Projects
										</h2>
										<ul className="-mx-4 mt-2 text-gray-700 text-sm">
											{filteredProjects.map((project) => (
												<ComboboxOption
													as="li"
													key={project.id}
													value={project}
													className="group flex cursor-default select-none items-center px-4 py-2 data-focus:bg-indigo-600 data-focus:text-white data-focus:outline-hidden"
												>
													<FolderIcon
						...
						<div className="flex flex-wrap items-center bg-gray-50 px-4 py-2.5 text-gray-700 text-xs">
							Type{" "}
							<kbd
