...
export const repositoriesRelations = relations(
  ...
  ({ one, many }) => ({
    createdBy: one(users, {
      fields: [repositories.createdById],
      references: [users.id],
...
export const repositoryTagsRelations = relations(repositoryTags, ({ one }) => ({
  repository: one(repositories, {
    fields: [repositoryTags.repositoryId],
    references: [repositories.id],
  }),
  tag: one(tags, { fields: [repositoryTags.tagId], references: [tags.id] }),
}));

export const listRepositoryEntriesRelations = relations(
  listRepositoryEntries,
  ({ one }) => ({
    list: one(lists, {
      fields: [listRepositoryEntries.listId],
      references: [lists.id],
    }),
    repository: one(repositories, {
      fields: [listRepositoryEntries.repositoryId],
      references: [repositories.id],
    }),
  })
);
